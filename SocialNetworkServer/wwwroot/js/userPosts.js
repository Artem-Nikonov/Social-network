...
async function getPosts() {
    try {
        const response = await fetch(`/pagePosts/${pageId}?pageType=userPage&startPostId=${startPostId}`, {
            method: "GET",
            headers: {
...
async function delBtnClick() {
    ...
    try {
        const response = await fetch(`/pagePosts/delete/${postId}?pageType=userPage`, {
            method: "PATCH"
        });
